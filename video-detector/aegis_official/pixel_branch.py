...
# Official AEGIS code hard-coded an author-machine cache path. Resolve the same
# timm DINOv2 safetensors via Hugging Face Hub (or AEGIS_DINOV2_WEIGHTS).
DINOV2_HF_REPO = "timm/vit_base_patch14_dinov2.lvd142m"
DINOV2_HF_FILE = "model.safetensors"


def _resolve_dinov2_safetensors() -> str:
    import os
    from pathlib import Path

    override = os.environ.get("AEGIS_DINOV2_WEIGHTS", "").strip()
    if override:
        path = Path(override)
        if not path.is_file():
            raise FileNotFoundError(
                f"AEGIS_DINOV2_WEIGHTS does not exist: {override}"
            )
        return str(path)

    from huggingface_hub import hf_hub_download

    return hf_hub_download(repo_id=DINOV2_HF_REPO, filename=DINOV2_HF_FILE)
...
def _load_dinov2_224():
    ...
    # Agirlikları Hugging Face / override path'ten yukle (official resize logic)
    state_dict = load_file(_resolve_dinov2_safetensors())
